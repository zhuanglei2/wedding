...
(async()=>{
  const image=await loadImage(__dirname+'/rig-atlas.png'),scope={devicePixelRatio:1};
  ...
  const g={width:390,height:844,viewportWidth:390,left:0,actorWidth:109.2};
  for(const [i,p] of [.06,.24,.55,.66,.78,.90].entries()){
    const stage=createCanvas(390,844),ctx=stage.getContext('2d');
    const layer=createCanvas(390,844),rig=scope.WeddingCharacterRig.create(layer,image,math);
  ...
  fs.writeFileSync('/private/tmp/wedding-v1064-flight-poses.png',sheet.toBuffer('image/png'));
