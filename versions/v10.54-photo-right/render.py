...
# Remove only the sign-off typesetting layer by restoring its exact original
# paper pixels, before applying the existing photo-height translation.
paper = Image.open(ROOT / 'versions/v10.47-soft-gold-light/background-light.png').convert('RGB').resize(base.size, Image.Resampling.LANCZOS)
signoff_box = (0, 720 * 4, base.width, 740 * 4)
base.paste(paper.crop(signoff_box), (0, 720 * 4))
photo = ImageOps.exif_transpose(Image.open(SOURCE)).convert('RGB')
# Move the crop window another 2% left so the people appear
...
crop_left = round(photo.width * .16)
...
canvas.save(HERE / 'invitation-v10.54.png')
# Preview only; the HTML always loads the full-resolution lossless master above.
canvas.resize((700, round(canvas.height / 2)), Image.Resampling.LANCZOS).save(HERE / 'preview-v10.54.png')
