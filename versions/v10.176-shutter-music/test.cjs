...
(async()=>{
 ...
 const before=fs.readFileSync(path.join(__dirname,'../v10.175-music-load-order/index.html'),'utf8');
 assert.equal(html,before.replace('V10.175','V10.176').replace('src="../v10.165-mobile-handoff/opening-runtime.js"','src="opening-runtime.js"').replace('照片显现后播放','按下快门后播放'));
 ...
 // Drive the real camera renderer's cue branch (no independent elapsed timer).
 const runtime=fs.readFileSync(path.join(__dirname,'opening-runtime.js'),'utf8');
 const camera=runtime.slice(runtime.indexOf('/* Source: camera-story.js */'));
 const cssSource=camera.slice(camera.indexOf('  function css(state){'),camera.indexOf('  function complete('));
 assert(cssSource.includes("new CustomEvent('camera-shutter'"));
 assert(runtime.includes('get shutterFired(){return shutterFired}'));
 const constants=runtime.match(/const FLASH_PEAK=\.78,FLASH_START=\d+[^;]+;/)[0];
 const flashStart=vm.runInNewContext(constants+' FLASH_START;');
 function clock(f){
  const context={document:f.doc,story:{FLASH_START:flashStart},stages:[],paper:null,flash:{style:{}},setStyle(){},CustomEvent:class{constructor(type,options){this.type=type;this.detail=options?.detail}}};
  vm.runInNewContext('let active=true,shutterFired=false;'+cssSource+';this.paint=css;this.cancel=()=>{active=false};',context);
  return context;
 }
 const m=fixture({start:18});let shutters=0;m.doc.addEventListener('camera-shutter',()=>shutters++);
 m.doc.fire('camera-story-started');const c1=clock(m);
 c1.paint({t:0,flash:0});c1.paint({t:flashStart-1,flash:0});
 assert.equal(m.audio.plays.length,0,'approach/prebuffer must not play the song');
 assert(!m.art.classList.contains('camera-photo-settled'));
 c1.paint({t:flashStart+1,flash:.01});await flush();
 assert.equal(m.audio.plays.length,1,'real shutter cue starts music before the photograph settles');
 assert.equal(m.audio.currentTime,18);assert.equal(m.player.dataset.state,'playing');
 m.toggle.fire('click');c1.paint({t:flashStart+100,flash:.78});m.ready();m.doc.fire('camera-story-complete');
 await flush();assert(m.audio.paused);assert.equal(m.audio.plays.length,1,'photo completion/duplicate cues must respect pause');assert.equal(shutters,1);
 const n=fixture({start:18,blocked:true}),c2=clock(n);
 c2.paint({t:flashStart+900,flash:0});await flush();assert.equal(n.player.dataset.state,'blocked','dropped flash frames still trigger only once');
 n.audio.block=false;n.toggle.fire('click');await flush();assert.equal(n.audio.currentTime,18);assert.equal(n.player.dataset.state,'playing');
 const o=fixture({start:18}),c3=clock(o);c3.cancel();c3.paint({t:undefined,flash:0,done:true});
 assert.equal(o.audio.plays.length,0,'cancelled render must not emit a fake shutter');o.ready();await flush();assert.equal(o.audio.plays.length,1,'static/fallback still starts on visible photo');
 const p=fixture({start:18});p.doc.hidden=true;p.doc.fire('camera-shutter');assert.equal(p.audio.plays.length,0);
 p.doc.hidden=false;p.doc.fire('visibilitychange');await flush();assert.equal(p.audio.plays.length,1,'hidden cue is deferred until visible');
 const q=fixture({start:18}),c4=clock(q);q.audio.pendingPlay=true;c4.paint({t:flashStart+1,flash:.01});
 assert.equal(q.player.dataset.state,'loading');q.ready();q.doc.fire('camera-story-complete');assert.equal(q.audio.plays.length,1);
 q.toggle.fire('click');q.audio.resolve();await flush();assert(q.audio.paused,'slow loading does not restart after cancel');
 console.log('PASS: actual shutter-clock cue → music at 18s before photo; pre-shutter silence; one shot; skipped frames; cancellation/static/hidden fallback; autoplay denial/manual play; slow request and user pause.');
