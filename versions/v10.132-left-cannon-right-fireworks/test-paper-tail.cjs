...
// New markup and delivery assets are checked by test-keepsake-loading.cjs.
for(const name of ['handoff-math.js']){
 assert.equal(read(__dirname,name),read(prior,name),name+' remains byte-identical');
}
const rules=css=>css.match(/\.camera-caption,.camera-names\{[\s\S]*?\.camera-caption::after\{[^}]+\}/)[0];
assert.equal(rules(css).replaceAll('../v10.125-keepsake-fast/media/camera-art.webp','../v10.111-camera-invitation/camera-invitation.png'),rules(read(prior,'camera-story.css')),'Bilingual caption unchanged; names/date isolation tested separately');
...
function setup(w,h,withObserver=true){
 ...
 const page={clientHeight:Math.max(h,art.offsetHeight),querySelector:()=>art,style:{setProperty:(k,v)=>values[k]=v},classList:{add:v=>classes.add(v),toggle:(v,on)=>on?classes.add(v):classes.delete(v)}};
...
for(const w of [320,375,390,430,768,1000])for(const h of [568,700,844,1100,1600]){
 ...
 assert.equal(s.classes.has('paper-has-notes'),false,'V126 removes the extra margin notes');
