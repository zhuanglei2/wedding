...
assert.equal(rules(css).replaceAll('media/camera-art.webp','../v10.111-camera-invitation/camera-invitation.png'),rules(read(prior,'camera-story.css')),'Names/date and bilingual caption unchanged');
...
for(const w of [320,375,390,430,768,1000])for(const h of [568,700,844,1100,1600]){
 ...
 assert.equal(s.classes.has('paper-has-notes'),s.art.offsetTop>=88&&gap>=88,'Notes only occupy spacious margins');
