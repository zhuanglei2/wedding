...
old=here.parent/'v10.60-star-trail'
html=(here/'index.html').read_text()
old_html=(old/'index.html').read_text()
for name in ('cover.webp','cover.jpg','classic-reveal.webp','classic-reveal.jpg','titles.css','star-couple.png','star-couple-reference.jpg','page-turn-math.js','star-trail.js'):
...
assert cover(html).replace(entry,'')==cover(old_html),'First invitation changed'
assert '<dialog' not in html and 'intro.js' not in html
assert 'V10.61' in html
assert html.replace(entry,'').split('<main>',1)[1].split('</main>',1)[0]==old_html.replace(cue,'').split('<main>',1)[1].split('</main>',1)[0], 'Unexpected invitation/chapter markup changes'
