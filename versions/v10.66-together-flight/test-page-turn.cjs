...
for(const width of [320,390,768,1000]){
  ...
  assert.ok(Math.abs(waitStart.x-waitEnd.x)<1e-7&&Math.abs(waitStart.y-waitEnd.y)<4,'Boy hovers instead of leaving alone');
  ...
  const joined=math.sample(math.DEPART_AT,screen).actors.map(center);
  for(const p of [.82,.88,.94,.98]){
    const pair=math.sample(p,screen).actors.map(center);
    assert.ok(Math.abs((pair[1].x-pair[0].x)-(joined[1].x-joined[0].x))<1e-7,'Shared horizontal flight');
    assert.ok(Math.abs((pair[1].y-pair[0].y)-(joined[1].y-joined[0].y))<1e-7,'Shared vertical flight');
  }
  for(const boundary of [.60,.64,.71,.79,.80])for(const i of [0,1]){
...
s.flush(3000);s.flush(5000);assert.equal(s.calls.poses.at(-1),1);assert.ok(!s.root.classes.has('turn-playing'));assert.equal(s.queue.size,0);
