/* V10.169: real audio, one photo-ready trigger, reader-owned playback. */
(()=>{'use strict';
 ...
 const back=player.querySelector('.music-back'),forward=player.querySelector('.music-forward');
 ...
 function state(value,message){
  ...
  const playing=value==='playing';icon.textContent=playing?'Ⅱ':'▶';
  toggle.setAttribute('aria-pressed',String(playing));
  toggle.setAttribute('aria-label',playing?'暂停音乐':value==='loading'?'取消音乐加载':'播放音乐');
 }
 function progress(){
  const total=length(),time=Number(audio.currentTime)||0;
  ...
  seek.disabled=back.disabled=forward.disabled=!ready||!total;
 }
 ...
 function stop(){wanted=false;resumeAfterHidden=false;request++;audio.pause();if(ready)state('paused','点击播放，拖动试听')}
 function attempt(){
 ...
 function rejected(error,id){
  ...
  else if(error?.name==='AbortError')state('paused','点击播放，拖动试听');
 ...
 back.addEventListener('click',()=>{interact();seekTo((audio.currentTime||0)-10)});
 forward.addEventListener('click',()=>{interact();seekTo((audio.currentTime||0)+10)});
 ...
 audio.addEventListener('pause',()=>{if(ready&&!priming&&audio.paused&&player.dataset.state!=='blocked'&&player.dataset.state!=='error')state('paused',document.hidden?'音乐已暂停':'点击播放，拖动试听')});
