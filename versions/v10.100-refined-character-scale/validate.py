...
old=here.parent/'v10.71-collar-and-motion'
html=(here/'index.html').read_text()
old_html=(old/'index.html').read_text()
for name in ('cover.webp','cover.jpg','classic-reveal.webp','classic-reveal.jpg','titles.css','star-couple.png','star-couple-reference.jpg','rig-atlas.png','rig-atlas-original.png','head-expressions.png','character-rig.js','page-turn.js','motion.css'):
...
assert html.replace('V10.100','V10.71')==old_html, 'Unexpected invitation/chapter markup changes'
assert (here/'paper-surface.js').read_text().replace('g.width*12','g.width*2.8')==(old/'paper-surface.js').read_text(),'Only paper perspective changes; original photo slices remain intact'
css=(here/'page-turn.css').read_text()
assert css.replace('drop-shadow(0 3px 3px #1809032e)','drop-shadow(0 5px 4px #18090344)')==(old/'page-turn.css').read_text(), 'Only the character shadow should change in page CSS'
