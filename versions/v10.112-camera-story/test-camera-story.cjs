...
for(const width of [320,375,390,430,760,1000])for(const height of [568,700,844,1100]){
  ...
  for(let t=0;t<=story.DURATION;t+=10){
    ...
    if(t<=story.PRINT_END){assert.equal(s.caption,0);assert.equal(s.names,0);}
    if(s.caption>0||s.names>0)assert.equal(s.print,1,'Text only AFTER full print');
    if(t>=2650&&t<4700){
    ...
    assert.ok(s.flash>=0&&s.flash<=.16);
  }
  ...
  assert.equal(last.print,1);assert.equal(last.caption,1);assert.equal(last.names,1);assert.equal(last.flash,0);assert.equal(last.opacity,0);
...
console.log('PASS: 24 viewport geometries, continuous handoff/flight, shutter contact, held-hand departure, single low-opacity flash, print-before-caption-before-names, original photo intact and resources.');
