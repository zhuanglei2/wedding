(function(scope){
  'use strict';
  const DURATION=5400,PRINT_START=1800,PRINT_END=3800,CAPTION_START=3950,NAMES_START=4350;
  ...
  function sample(ms,plan){
    ...
    const flash=t>=1600&&t<=1800?Math.sin((t-1600)/200*Math.PI)*.16:0;
    return{t,p:t>=DURATION?1:Math.max(.001,t/DURATION),actors,emitters:actors.map(a=>a.emitter),opacity:Math.max(...actors.map(a=>a.opacity)),
      flash,press:pressDip/3,print:ease((t-PRINT_START)/(PRINT_END-PRINT_START)),
      caption:ease((t-CAPTION_START)/800),names:ease((t-NAMES_START)/850),done:t>=DURATION};
  }
  const api={makePlan,sample,hand,center,DURATION,PRINT_START,PRINT_END,CAPTION_START,NAMES_START};
