...
async function setup(options={}){
  ...
  const root=new El(),body=new El(),stage=new El(),mirror=new El(),rigImage=new El(),eyes=new El(),replay=new El(),status=new El(),reduce=new El();
  ...
  const nodes={'.rig-source':rigImage,'.head-source':eyes,'[data-camera-replay]':replay,'[data-camera-status]':status,'#our-story .reference-art':stage};
  ...
  return{win,doc,stage,mirror,replay,root,reduce,draws,emit,tick,queue,timers,status};
}
(async()=>{
  let s=await setup();assert.ok(s.win.WeddingCameraStory.ready);assert.ok(s.win.WeddingCameraStory.active);
  ...
  s.tick(4100);assert.ok(+s.stage.values['--caption-alpha']>0);assert.equal(s.stage.values['--names-alpha'],'0');
  s.tick(5400);assert.equal(s.stage.values['--names-alpha'],'1');assert.equal(s.queue.size,0);assert.ok(!s.win.WeddingCameraStory.active);
  assert.equal(s.win.WeddingCameraStory.start(),false);s.replay.emit('click');assert.ok(s.win.WeddingCameraStory.active);s.win.WeddingCameraStory.complete();
  for(const type of ['visibilitychange','resize','pagehide','Escape','Tab','scroll','multiTouch']){
    s=await setup();s.tick(0);s.tick(1700);
    ...
    assert.ok(!s.win.WeddingCameraStory.active,type);assert.equal(s.queue.size,0);assert.equal(s.stage.values['--print-y'],'0%');assert.equal(s.stage.values['--names-alpha'],'1');
  }
  for(const options of [{reduce:true},{broken:true},{noCanvas:true}]){
    s=await setup(options);assert.ok(!s.win.WeddingCameraStory.active);assert.equal(s.queue.size,0);assert.equal(s.stage.values['--names-alpha'],'1');assert.equal(s.stage.values['--camera-clean'],'0');
  }
  ...
  console.log('PASS: actual runtime auto preview/replay, production start, synchronized layers, full-print-before-copy, interruption cleanup, error/reduced-motion fallback, duplicate click and native scroll recovery.');
