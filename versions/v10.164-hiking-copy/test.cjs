...
async function fixture({reduced=false,noWAAPI=false,broken=false,paperHeight=850,screenHeight=850,introHeight=280,wrap=18,rowHeight=28,scrollQuantum=0,frameStep=32,sourceOverride=null,handoffPending=false,integrated=false}={}){
 ...
 if(integrated){
  ...
  vm.runInNewContext(fs.readFileSync(path.join(__dirname,'../v10.163-love-copy-handoff/memory-handoff.js'),'utf8'),context);
 }
...
(async()=>{
 ...
 console.log('PASS: V10.159 freeze reproduced; V10.164 preserves the V10.160 fix with integer/half/third-pixel scrolling, 8/16/32ms frames, both directions, clamped scroll bounds and photo flights.');
