...
def package(prefix):
    ...
    while pending:
        ...
        data = raw[file]
        if file.suffix == '.css':
            if '@import' in data.decode(): raise RuntimeError('CSS imports require explicit dependency handling')
...
PREFIX = 'previews/v10.156-20260911-' + fingerprint[:12] + '/'
...
assert b'V10.156' in html and b'href="#our-story"' in html
assert html.count(b'data-memory-card=') == 19
assert '文涛厅'.encode() in html and '这一刻的幸福<br>想与你一同分享'.encode() in html
...
plan = dict(version='V10.156', baseRevision=REV, sourceFingerprint=fingerprint, bucket=BUCKET,
...
def request(method, key, data=None, kind='', cache=None, optional=False, create_only=False):
