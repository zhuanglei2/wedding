...
async function fixture({reduced=false,noWAAPI=false,broken=false,paperHeight=850,screenHeight=850,introHeight=280,wrap=18,rowHeight=28,scrollQuantum=0,frameStep=32,sourceOverride=null,handoffPending=false}={}){
...
(async()=>{
 ...
 console.log('PASS: V10.159 freeze reproduced; V10.162 preserves the V10.160 fix with integer/half/third-pixel scrolling, 8/16/32ms frames, both directions, clamped scroll bounds and photo flights.');
 ...
 for(const options of [{paperHeight:640,screenHeight:640,wrap:8,rowHeight:34},{paperHeight:850,screenHeight:850},{paperHeight:900,screenHeight:600,wrap:12,rowHeight:32}]){
  ...
  await reading.to(plan.end+300);assert(checked>100&&centered>5,`faster line-by-line tracking exercised: ${checked} visible, ${centered} centered`);
 }
 ...
 console.log('PASS: faster glyph timing; third-page handoff gate; unchanged copy/layout/photo schedule; 19 cards land before hold/fade; centered keepsake; enlarged finale; line-centered scrolling; font reflow/resize; 1000ms holds; fixed comic/paper; no document scroll; 640/850/900/1500px heights; once only; manual override; reduced motion; image-error fallback.');
