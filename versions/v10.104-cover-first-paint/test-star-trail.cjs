...
assert.ok(live.every(p=>p.opacity>=0&&p.opacity<=.74&&Number.isFinite(p.x)&&Number.isFinite(p.y)));
assert.ok(live.every(p=>p.radius<=(p.star?6.4:2.3)*.8),'Trail is subordinate to the smaller characters');
