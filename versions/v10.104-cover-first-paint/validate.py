...
old=here.parent/'v10.103-led-departure'
html=(here/'index.html').read_text()
old_html=(old/'index.html').read_text()
for name in ('cover.webp','cover.jpg','classic-reveal.webp','classic-reveal.jpg','titles.css','star-couple.png','star-couple-reference.jpg','rig-atlas.png','rig-atlas-original.png','head-expressions.png','motion.css','paper-surface.js','star-trail.js','page-turn.js','page-turn.css','page-turn-math.js','character-rig.js','rig-atlas.webp','eye-lids.png'):
...
assert 'V10.104' in html
restored=html.replace('V10.104','V10.103')
for name in ('motion.css','titles.css','page-turn.css'):
    block='<style data-inline-source="'+name+'">\n'+(here/name).read_text()+'</style>'
    assert restored.count(block)==1,'Inline CSS must remain byte-identical: '+name
    restored=restored.replace(block,'<link rel="stylesheet" href="'+name+'">')
font_line=next(line for line in old_html.splitlines() if line.startswith('<link rel="stylesheet" href="https://fonts.'))
restored=restored.replace('<noscript>',font_line+'\n<noscript>',1)
restored=re.sub(r'<script id="after-cover-font">.*?</script>\n','',restored,flags=re.S)
assert restored==old_html,'Only inline CSS, delayed font and version labels may change'
...
assert html[html.index('<body>'):html.index('</main>')]==old_html[old_html.index('<body>'):old_html.index('</main>')], 'Visible invitation/chapter markup must remain unchanged'
