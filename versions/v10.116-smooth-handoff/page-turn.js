(() => {
  ...
  function paint(timestamp){
    ...
    try{
      ...
      // Continue release/rejoining instead of freezing at .79 for the final 1.3 seconds.
      const held=math.sample(bridge.actorProgress(progress),geometry);
  ...
  function finish(focus=false,skipStory=false,timestamp){
    ...
    else window.WeddingCameraStory?.start({actors:handoff,view:bridge.cameraView(geometry),timestamp});
  }
  ...
  function configure(){
    ...
    try{
      ...
      const actorWidth=math.actorWidth(width),storyHeight=chapter.querySelector('.reference-art')?.offsetHeight||width*1.5;
      const readingOffset=Math.max(0,sheetHeight-height);
      geometry={width,height,storyHeight,left:rect.left,viewportWidth:window.innerWidth,actorWidth,actorHeight:actorWidth*340/240,start:rect.top+window.scrollY+readingOffset,distance};
