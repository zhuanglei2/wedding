(() => {
  ...
  function css(state){
    ...
    for(const node of stages)for(const [key,value] of Object.entries(values))setStyle(node,key,value);
  ...
  function start(input={}){
    ...
    plan=story.makePlan(math,g,rect,input.actors);active=true;played=true;started=Number.isFinite(input.timestamp)?input.timestamp:null;scrollAt=window.scrollY;
