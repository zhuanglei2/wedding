...
change('const win={WeddingPageTurn:math,','const win={WeddingPageTurn:math,WeddingHandoffMath:require("./handoff-math.js"),');
change('calls.rig++;calls.poses.push(state.p);','calls.rig++;calls.poses.push(state.p);calls.lastActors=state.actors;');
...
source+=`
s=setup();s.nodes['.cover-enter'].emit('click',{preventDefault(){}});s.flush(0);s.flush(2600);const girlA=s.calls.lastActors[1];s.flush(3400);const girlB=s.calls.lastActors[1];assert.ok(Math.hypot(girlB.x-girlA.x,girlB.y-girlA.y)>.1,'No frozen 1.3-second interlude');
...
assert.equal(s.calls.handoff.timestamp,3800);assert.equal(s.calls.handoff.view.rect.top,0);assert.equal(s.calls.handoff.view.rect.height,585);
