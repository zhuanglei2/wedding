...
(function(scope){
  ...
  function paper(p,g){
    ...
    for(let i=0;i<SEGMENTS;i++){
      // CSS negative rotateY lifts the right edge toward the viewer (+Z).
      // Keep both the strip transform and the accumulated spine signed alike.
  ...
  function actor(p,g,sheet,index){
    ...
    // A small depth approach follows the lifted paper without changing the
    // fixed hand contact: its full rotated/scaled endpoint is subtracted below.
    const scale=g.actorWidth/240*(1+.16*clamp(sheet.z/g.width)*grip);
