...
for(const width of [320,390,768,1000]){
  const screen={...g,width,viewportWidth:width,actorWidth:math.actorWidth(width)};
  assert.ok(Math.abs(screen.actorWidth/(Math.min(150,Math.max(95,width*.28)))-.68)<1e-9);
...
function setup(opts={}){
  ...
  if(opts.coverUnloaded)nodes['.image-cover'].image.complete=false;
  const decodeQueue=[];
  if(opts.decode)for(const image of [nodes['.image-cover'].image,nodes['.rig-source'],nodes['.head-source'],nodes['.opening-underlay img']]){
    image.decode=()=>({then:resolve=>decodeQueue.push(resolve)});
  }
  if(opts.deferred)for(const [selector,path] of [['.rig-source','rig-atlas.webp'],['.head-source','eye-lids.png']]){
    const image=nodes[selector];image.complete=false;image.naturalWidth=0;
    image.getAttribute=name=>name==='data-src'?path:null;
  }
  ...
  return{nodes,root,win,queue,events,reduce,doc,calls,flush,decodeQueue};
}
...
s.flush(0);s.flush(350);s.flush(2500);assert.equal(s.win.scrollY,0,'Click plays in place without a bottom alignment scroll');
...
// First visit: don't compete with the cover or prematurely enable the canvas.
s=setup({coverUnloaded:true,deferred:true});
assert.equal(s.nodes['.rig-source'].src,undefined);
assert.equal(s.nodes['.head-source'].src,undefined);
assert.ok(!s.root.classes.has('star-turn'));
s.nodes['.image-cover'].image.complete=true;s.nodes['.image-cover'].image.emit('load');
assert.equal(s.nodes['.rig-source'].src,'rig-atlas.webp');
assert.equal(s.nodes['.head-source'].src,'eye-lids.png');
assert.ok(!s.root.classes.has('star-turn'),'Wait for real sprite pixels');
for(const selector of ['.rig-source','.head-source']){const image=s.nodes[selector];image.complete=true;image.naturalWidth=80;image.emit('load');}
s.flush();assert.ok(s.root.classes.has('star-turn'),'Enable only after cover and animation resources are ready');
s=setup({reduce:true,deferred:true});
assert.equal(s.nodes['.rig-source'].src,undefined,'Reduced motion does not fetch deferred sprites');
console.log('PASS: cover-first deferred sprite loading, ready gate and reduced-motion resource savings');
s=setup({decode:true});assert.equal(s.decodeQueue.length,4);assert.ok(!s.root.classes.has('star-turn'));
for(const complete of s.decodeQueue)complete();s.flush();
assert.ok(s.root.classes.has('star-turn'),'All unique textures decoded before animation activation');
console.log('PASS: parallel unique texture decode gate with load-event fallback');

// Exact tap crop is preserved at top/middle/bottom, in short and long pages.
for(const [width,viewport,coverHeight,left] of [[320,650,750,0],[390,700,914,0],[390,844,914,0],[390,1000,914,0],[768,844,1800,116]]){
 for(const fraction of [0,.5,1]){
  s=setup();s.win.innerHeight=viewport;s.win.innerWidth=width+left*2;
  s.nodes['.opening-sequence'].clientWidth=width;s.nodes['.image-cover'].offsetHeight=coverHeight;
  s.nodes['.opening-sequence'].getBoundingClientRect=()=>({top:-s.win.scrollY,left});
  const readingOffset=Math.max(0,coverHeight-viewport);
  s.nodes['.image-cover'].getBoundingClientRect=()=>({top:-Math.min(s.win.scrollY,readingOffset),left,width});
  emit(s,'resize');s.flush();
  const readY=readingOffset*fraction;s.win.scrollY=readY;
  const before=s.nodes['.image-cover'].getBoundingClientRect();
  const scrolls=[],nativeScroll=s.win.scrollTo;s.win.scrollTo=o=>{scrolls.push(o.top);nativeScroll(o);};
  s.nodes['.cover-enter'].emit('click',{preventDefault(){}});
  assert.equal(parseFloat(s.root.values.get('--play-sheet-top')),before.top || 0);
  assert.equal(parseFloat(s.root.values.get('--play-sheet-left')),left);
  assert.equal(parseFloat(s.root.values.get('--play-sheet-width')),width);
  const hinge=parseFloat(s.nodes['.turn-sheet'].values.get('--hinge-y'));
  assert.ok(Math.abs(hinge+before.top-viewport*.60)<1e-8,'Same viewport hinge for paper and hands');
  s.flush(0);s.flush(16);
  assert.ok(s.calls.poses.at(-1)>0,'Animation starts immediately, no 250ms pre-scroll');
  for(const time of [80,240,500,2000,5000,math.DURATION-1]){s.flush(time);assert.equal(s.win.scrollY,readY);}
  assert.equal(scrolls.length,0,'No page-scroll writes during a normal tap animation');
  s.flush(math.DURATION);
  assert.deepEqual(scrolls,[coverHeight],'One final handoff after the paper is gone');
  for(const name of ['--play-sheet-top','--play-sheet-left','--play-sheet-width'])assert.ok(!s.root.values.has(name));
  assert.ok(!s.nodes['.turn-sheet'].values.has('--hinge-y'));
  assert.ok(!s.root.classes.has('turn-playing'));assert.equal(s.queue.size,0);
 }
}
s=setup();s.win.scrollY=85;emit(s,'scroll');s.flush(0);s.win.scrollY=210;emit(s,'scroll');
assert.equal(s.win.scrollY,85,'Residual mobile momentum cannot drag the background');
s.flush(400);assert.ok(s.calls.poses.at(-1)>0,'Momentum correction does not cancel animation');
emit(s,'keydown',{key:'Escape',preventDefault(){}});
assert.ok(!s.root.values.has('--play-sheet-top'),'Escape clears fixed positioning');
const playCSS=fs.readFileSync(__dirname+'/page-turn.css','utf8');
assert.ok(playCSS.includes('html.star-turn.turn-playing .turn-sheet{position:fixed;top:var(--play-sheet-top);left:var(--play-sheet-left);width:var(--play-sheet-width)'));
assert.ok(playCSS.includes('height:var(--track-height)'),'Track retains original document flow');
console.log('PASS: 15 tap positions preserve viewport crop/hinge, immediate animation, zero in-flight scroll writes, one final handoff, momentum/escape cleanup');
