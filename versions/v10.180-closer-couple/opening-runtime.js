/* V10.132: ordered deferred runtime bundle. */
;
...
(function(scope){
  ...
  function create(container,coverImage,count){
    const fronts=[],backs=[],strips=[],photoLayers=[],photoImages=[];
    const photoTemplate=coverImage.closest('.image-cover')?.querySelector('.cover-photo-reframe');
    for(let i=0;i<count;i++){
      ...
      // Only the strips intersecting the static photo window need the crop.
      // Reuse the cover's chosen resource: no extra photo download or AI edit.
      let photoLayer=null;
      if(photoTemplate&&(i+1)/count>264/1400&&i/count<1136/1400){
        photoLayer=photoTemplate.cloneNode(true);
        photoLayer.querySelectorAll('source').forEach(source=>source.remove());
        const photoImage=photoLayer.querySelector('img');
        photoImage.removeAttribute('srcset');
        photoImage.removeAttribute('sizes');
        photoImage.src=coverImage.currentSrc||coverImage.src;
        front.appendChild(photoLayer);photoImages.push(photoImage);
      }
      photoLayers.push(photoLayer);
      strip.appendChild(front);strip.appendChild(back);container.appendChild(strip);
      fronts.push(img);backs.push(back);strips.push(strip);
    }
    return {
      images:fronts.concat(photoImages),
      configure(g){
        ...
        strips.forEach((strip,i)=>{
          ...
          if(photoLayers[i]){
            photoLayers[i].style.width=g.width+'px';
            photoLayers[i].style.height=g.width*3282/1400+'px';
            photoLayers[i].style.left=-i*g.width/count+'px';
          }
...
(() => {
  ...
  let ready=false,active=false,played=false,failed=false,frame=0,started=null,plan=null,scrollAt=0,playbackWidth=0,playbackScale=1;
  ...
  function css(state){
    ...
    // Emit once from the actual animation clock, including frames that skip the flash peak.
    // complete() clears active first, so a skipped/cancelled scene does not fake a shutter.
    if(active&&!shutterFired&&state.t>=story.FLASH_START){
      shutterFired=true;
      document.dispatchEvent(new CustomEvent('camera-shutter',{detail:{time:state.t}}));
    }
  }
  function complete(reason='cancelled'){
    ...
    document.dispatchEvent(new CustomEvent('camera-story-complete',{detail:{reason}}));
  }
  function geometry(){
  ...
  function start(input={}){
    ...
    plan=story.makePlan(math,g,rect,input.actors,input.flight);active=true;played=true;started=Number.isFinite(input.timestamp)?input.timestamp:null;scrollAt=window.scrollY;playbackWidth=window.innerWidth;playbackScale=window.visualViewport?.scale||1;
    ...
    const draw=timestamp=>{
      ...
      if(state.done){complete('finished');return;}
  ...
  window.WeddingCameraStory={start,complete,get ready(){return ready&&!failed&&!preference.matches},get active(){return active},get played(){return played},get shutterFired(){return shutterFired}};
  ...
  window.addEventListener('scroll',()=>{
    if(active&&Math.abs(window.scrollY-scrollAt)>8)complete();
