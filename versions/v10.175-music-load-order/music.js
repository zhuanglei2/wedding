/* V10.175: visual assets first; defer the unchanged MP3 until the invitation opens. */
(()=>{'use strict';
 ...
 document.addEventListener('camera-assets-ready',()=>{assetsReady=true;bufferWhenReady();checkPhoto()});
 ...
 document.addEventListener('visibilitychange',()=>{
  ...
  else{bufferWhenReady();checkPhoto();if(resumeAfterHidden&&wanted){resumeAfterHidden=false;attempt()}}
 });
 window.addEventListener('pagehide',()=>{resumeAfterHidden=ready&&wanted;request++;audio.pause()});
 window.addEventListener('pageshow',()=>{bufferWhenReady();checkPhoto();if(resumeAfterHidden&&wanted&&!document.hidden){resumeAfterHidden=false;attempt()}});
 checkPhoto();
})();
