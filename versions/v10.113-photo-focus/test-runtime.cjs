...
(async()=>{
  ...
  assert.equal(s.win.WeddingCameraStory.start(),false);s.replay.emit('click');assert.ok(s.win.WeddingCameraStory.active);s.tick(0);assert.equal(s.stage.values['--photo-scale'],'1');assert.equal(s.stage.values['--camera-hide'],'0');
  ...
  console.log('PASS: actual runtime auto preview/replay, production start, synchronized layers, full-print-before-focus-before-copy and retained final zoom, interruption cleanup, error/reduced-motion fallback, duplicate click and native scroll recovery.');
