...
(async()=>{
  ...
  for(const [column,p] of [.12,.34,.57].entries()){
    const layer=createCanvas(1080,650),rig=scope.WeddingCharacterRig.create(layer,source,math,heads);
  ...
  fs.writeFileSync('/private/tmp/wedding-v1070-rig-poses.png',output.toBuffer('image/png'));
