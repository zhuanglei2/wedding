...
(function(scope){
  ...
  function holdHands(p,g,actors){
    ...
    const anchor={x:g.left+g.width*.5+g.width*.72*departure*departure,
      y:g.height*.465-(g.height*.64+g.actorWidth)*departure*departure+3*Math.sin(clamp((p-JOIN_AT)/.06)*Math.PI)};
    return actors.map(a=>{
  ...
  function expressions(p,g,actors){
    ...
    return actors.map((a,i)=>{
      ...
      a.eyeOpen=1-.96*Math.max(blink(i===0?.244:.278),blink(i===0?.6625:.7035),blink(i===0?.812:.835,i===0?.032:.050));
      ...
      const turn=ease((p-(i===0?.641:.682))/.075);
      ...
      a.headAngle=mix((i===0?7:-7)*turn+(i===0?5:-4)*acknowledgement,-13,launch)-3*pull;
      a.headNod=6*acknowledgement-6*launch;
      a.headShift=3*a.headYaw;
      ...
      a.clothLag=17*lagLaunch+7*pull-5*settle;
      a.clothRipple=3*Math.sin(p*32+i)*Math.sin(clamp((p-.20)/.80)*Math.PI);
      a.torsoTurn=(i===0?1:-1)*turn*(1-away);
      a.elbowBend=7*turn*(1-launch)+4*pull;
      a.legFollow=12*lagLaunch-6*settle;
