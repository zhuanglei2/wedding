...
(()=>{'use strict';
 ...
 function canPlay(){return !done&&!document.hidden&&!pointerHeld&&!touchHeld&&Date.now()>=gestureUntil&&visible(port)&&!root.classList.contains('turn-playing')&&!root.classList.contains('paper-motion')}
 ...
 function restoreFollow(){
  follow=true;rejoining=true;geometry=null;port.classList.add('story-following');
  if(activeCursor&&section.dataset.storyPhase==='typing')trackLine(...activeCursor);
 ...
 function tick(now){frame=0;if(!canPlay()){lastTime=null;return}const dt=lastTime===null?0:Math.min(64,now-lastTime);lastTime=now;
  // After manual reading, ease back to the paused line before advancing its clock.
  if(rejoining){
   if(scrollTarget===null||Math.abs(scrollTarget-port.scrollTop)<1){scrollTarget=null;rejoining=false}
   else port.scrollTop+=(scrollTarget-port.scrollTop)*Math.min(1,dt/140);
   queue();return;
  }
  ...
  if(!waitingForPhotos)elapsed+=dt;
   ...
   if(follow&&scrollTarget!==null){const distance=scrollTarget-port.scrollTop;if(Math.abs(distance)<1)scrollTarget=null;else port.scrollTop+=distance*Math.min(1,dt/140)}
   keepLineVisible();
   renderPhotos();
   if(elapsed>=plan.end){done=true;port.classList.remove('story-following');phase('complete');return}
  queue();
 }
