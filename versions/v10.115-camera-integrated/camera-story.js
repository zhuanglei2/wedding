(() => {
  ...
  function css(state){
    for(const node of stages){
      ...
      node.style.setProperty('--photo-scale',String(state.photoScale));
      node.style.setProperty('--photo-rise',state.photoRise+'%');
      node.style.setProperty('--camera-hide',String(state.cameraHide));
      node.style.setProperty('--lens-flash',String(state.lensFlash));
    }
