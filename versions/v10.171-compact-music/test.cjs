...
const html=fs.readFileSync(path.join(__dirname,'index.html'),'utf8'),css=fs.readFileSync(path.join(__dirname,'music.css'),'utf8');
assert(html.includes('data-start-seconds="50"'));
...
function fixture({blocked=false,start=0,loaded=true,visible=true,settled=false,pending=true}={}){
 ...
 return{doc,win,root,art,photo,cover,player,audio,toggle,seek,elapsed,status,options,mutate,ready:()=>{art.classes.add('camera-photo-settled');mutate()}};
}
(async()=>{
 ...
 const e=fixture({start:50});e.ready();await flush();assert.equal(e.audio.currentTime,50);e.audio.ended=true;e.audio.fire('ended');e.toggle.fire('click');await flush();assert.equal(e.audio.currentTime,50,'configured replay starts at chosen offset');
 ...
 console.log('PASS: no initial audio request; silent gesture prep; photo-loaded/visible/settled gate; blocked autoplay/manual fallback; real playback state; seek/disclosure; once only; background/manual pause; retry; 50-second offset; late-promise cancellation.');
