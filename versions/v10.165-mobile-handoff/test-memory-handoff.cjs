...
const script=fs.readFileSync(path.join(__dirname,'memory-handoff.js'),'utf8');
function fixture({reduced=false,scrollY=1000,quantum=1}={}){
 ...
 const doc={...eventTarget(),documentElement:root,scrollingElement:root,hidden:false,querySelector:s=>s==='#celebration'?source:s==='#wedding-invitation'?target:null};
 doc.dispatchEvent=e=>{if(e.type==='memory-handoff-complete')finished.push(time);doc.fire(e.type,e)};
 ...
 return {win,doc,root,preference,moves,finished,advance,start(){doc.fire('camera-story-started')},complete(reason='finished'){doc.fire('memory-story-complete',{detail:{reason}})},get targetReads(){return targetReads},get pendingFrames(){return frames.size}};
}
for(const quantum of [1,.5,1/3]){
 const f=fixture({quantum});f.start();assert(!f.root.classes.has('memory-handoff-pending'));f.advance(6000);assert.equal(f.moves.length,0,'no scroll while timeline plays');
 f.complete();f.advance(999);assert.equal(f.moves.length,0,'full one-second hold');assert(f.root.classes.has('memory-handoff-pending'));
 f.advance(705);assert(f.win.scrollY>1000&&f.win.scrollY<2080,'smooth intermediate position');assert(f.root.classes.has('memory-handoff-pending'),'handoff stays active until arrival');
 f.advance(900);assert.equal(f.win.scrollY,2080);assert(!f.root.classes.has('memory-handoff-pending'));assert.equal(f.finished.length,1);assert.equal(f.pendingFrames,0);assert.equal(f.root.style.scrollBehavior,'smooth');
 assert.equal(f.targetReads,1,'geometry sampled once');assert(f.moves.every((m,i)=>!i||m.y>=f.moves[i-1].y),'only moves downward');
...
for(const type of ['wheel','touchstart','touchmove','pointerdown','keydown'])for(const at of [500,1500]){
 const f=fixture();f.start();f.complete();f.advance(at);f.win.fire(type,{key:'ArrowDown'});const y=f.win.scrollY,n=f.moves.length;f.advance(3000);assert.equal(f.win.scrollY,y);assert.equal(f.moves.length,n);assert(!f.root.classes.has('memory-handoff-pending'));assert.equal(f.pendingFrames,0);
}
for(const reason of ['cancelled','failed',undefined]){const f=fixture();f.start();f.doc.fire('memory-story-complete',{detail:{reason}});f.advance(4000);assert.equal(f.moves.length,0,'fallback is not natural playback completion')}
...
const drag=fixture();drag.start();drag.complete();drag.advance(1200);drag.win.scrollY+=100;drag.win.fire('scroll');const dragMoves=drag.moves.length;drag.advance(3000);assert.equal(drag.moves.length,dragMoves,'scrollbar/manual scroll stops auto movement');
...
console.log('PASS: natural completion only; exact 1s hold; finite monotonic downward scroll; final fourth-page anchor; one-second final photo hold; one-shot; user cancellation; visibility/reduced-motion/scrollbar/resize compatibility; no pixel-convergence lock.');
