...
(function(scope){
  ...
  function create(container,coverImage,count){
    const fronts=[],backs=[],strips=[],dateLayers=[];
    const dateSource=coverImage.closest?.('.image-cover')?.querySelector('.cover-date-revision');
    for(let i=0;i<count;i++){
      ...
      const dateLayer=dateSource?.cloneNode(true);
      if(dateLayer){front.appendChild(dateLayer);dateLayers.push(dateLayer)}
    ...
    return {
      ...
      configure(g){
        ...
        strips.forEach((strip,i)=>{
          ...
          if(dateLayers[i]){dateLayers[i].style.width=g.width+'px';dateLayers[i].style.height=g.width*3282/1400+'px';dateLayers[i].style.left=-i*g.width/count+'px'}
