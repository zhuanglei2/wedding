...
canvas.save(HERE/'invitation-v10.48.png', optimize=True)
canvas.save(HERE/'invitation-v10.48.jpg',quality=95,subsampling=0)
canvas.resize((700,1516),Image.Resampling.LANCZOS).save(HERE/'preview-v10.48.png',optimize=True)
# Optional reference board, with original reference content and watermarks left intact.
reference_path = ASSETS/'invitation-v10.47.png'
if reference_path.exists():
    ref = Image.open(reference_path).convert('RGB')
    difference = ImageChops.difference(canvas,ref)
    # Only the dinner heading and address line may change.
    ImageDraw.Draw(difference).rectangle((0,564*S,W,586*S),fill=0)
    ImageDraw.Draw(difference).rectangle((0,679*S,W,697*S),fill=0)
    ...
    labels.text((374,30),'V10.47 · 调整前',font=font,fill='#ece6de',anchor='mt')
    labels.text((1098,30),'V10.48 · 地址与晚宴',font=font,fill='#ece6de',anchor='mt')
    board.paste(ref,(24,68))
    board.paste(canvas.resize((700,1516),Image.Resampling.LANCZOS),(748,68))
    board.save(HERE/'comparison-v10.48.jpg',quality=94,subsampling=0)
    canvas.crop((35*S,558*S,315*S,702*S)).save(HERE/'details-v10.48.png')
