...
const bundle=fs.readFileSync(path.join(__dirname,'../v10.165-mobile-handoff/opening-runtime.js'),'utf8');
...
async function fixture(edge=2,{legacy=false,width=390,frameStep=16,reduced=false,scrollDelay=0}={}){
 ...
 win.scrollTo=({top})=>{moves.push(Math.round(top));const apply=()=>{win.scrollY=Math.round(top);win.fire('scroll')};if(scrollDelay)timers.set(++id,{f:apply,at:now+scrollDelay});else{win.scrollY=Math.round(top);Promise.resolve().then(()=>win.fire('scroll'))}};
...
(async()=>{
 for(const scrollDelay of [16,64,200])for(const frameStep of [8,16,32]){
  const f=await fixture(2,{scrollDelay,frameStep});await f.advance(f.duration+4000);
  assert.equal(f.completions[0].reason,'finished');
  assert.equal(f.win.scrollY,2080,'actual camera completion feeds delayed-scroll handoff');
  assert.equal(f.pendingFrames,0);
 }
 console.log('PASS: actual bundled camera math + handoff, 9 asynchronous native-scroll scenarios.');
 ...
 for(const change of ['width','pinch','background','large-scroll'])for(const phase of ['camera','hold','scroll']){
  ...
  if(change==='large-scroll'){f.win.fire('pointerdown');f.win.scrollY+=100;f.win.fire('scroll');}
 ...
 console.log('PASS: manual touch/wheel/key/pointer, pinch, orientation, scrollbar drag, background and reduced motion keep cancellation behavior. No browser/device verification claimed.');
