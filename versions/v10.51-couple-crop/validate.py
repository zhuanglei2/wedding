...
old=(here.parent/'v10.50-seal-mobile/index.html').read_text()
start='<article class="chapter classic"'
assert html[html.index(start):].replace('V10.51','V10.50')==old[old.index(start):]
master=Image.open(here/'invitation-v10.51.png')
...
for asset in json.loads((here/'sizes.json').read_text()):
    ...
    with Image.open(file) as image:
        ...
        assert abs(image.height/image.width-master.height/master.width)<.002
