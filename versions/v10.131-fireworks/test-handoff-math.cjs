...
for(const width of [320,375,390,430,760,1000])for(const height of [568,700,844,1100]){
  ...
  assert.equal(view.rect.top,g.storyTop);assert.equal(view.rect.height,width*1.5);
  for(let t=0;t<=bridge.COVER_DURATION+bridge.CONTACT_AT;t+=8){
  ...
  for(const boundary of [500,650,720,1020,1150,1380,1450,1760,1950,2650,2850]){
  ...
  for(let t=1380;t<1450;t+=10)assert.ok(distance(camera.hand(camera.sample(t,plan).actors[0],true),plan.shutter)<1e-7,'Boy reaches actual shutter');
  ...
  const cover=width*3282/1400,poster=Math.max(height,width*1.5);
