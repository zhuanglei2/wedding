const assert=require('node:assert/strict'),fs=require('node:fs'),vm=require('node:vm'),math=require('../v10.104-cover-first-paint/page-turn-math.js'),story=require('./camera-story-math.js');
async function setup(options={}){
  class El{
    constructor(){this.events={};this.values={};this.writes=0;this.measures=0;this.attrs={};this.children=[];this.style={setProperty:(k,v)=>{this.writes++;this.values[k]=v}};this.classes=new Set();this.classList={add:n=>this.classes.add(n),remove:n=>this.classes.delete(n),contains:n=>this.classes.has(n)};this.complete=true;this.naturalWidth=100;this.disabled=true;}
    ...
    getBoundingClientRect(){this.measures++;return{left:0,top:0,width:390,height:585,bottom:585}}
  ...
  const events={},queue=new Map(),timers=new Map(),draws=[],paperDraws=[],paperPrepares=[],scrolls=[];let id=0,paperClears=0;
  const win={WeddingPageTurn:math,WeddingCameraMath:story,innerWidth:390,innerHeight:844,scrollY:0,scrollX:options.scrollX||0,visualViewport:{scale:options.zoom||1},scrollTo:p=>scrolls.push(p),matchMedia:()=>reduce,
    WeddingInstantPaper:{create:()=>({prepare:w=>paperPrepares.push(w),paint:s=>paperDraws.push(s),clear:()=>paperClears++})},
  ...
  return{win,doc,stage,mirror,root,reduce,draws,emit,tick,queue,timers,status,paperDraws,paperPrepares,scrolls,get paperClears(){return paperClears}};
}
(async()=>{
  ...
  s.tick(0);s.tick(3400);assert.ok(s.paperDraws.at(-1).paperBend>.4,'Production clock drives the visible curve');
  assert.ok(s.paperPrepares.every(w=>w===390),'Paper uses the same layout width as the actors');
  s.tick(3800);assert.equal(s.stage.values['--print-y'],'0%');assert.equal(s.stage.values['--caption-alpha'],'0');assert.equal(s.stage.values['--names-alpha'],'0');
  ...
  assert.ok(!s.stage.classes.has('camera-photo-settled'),'Do not flatten the image during zoom');
  ...
  assert.ok(s.stage.classes.has('camera-photo-settled'),'Flatten only after final focus is reached');
  ...
  assert.equal(s.paperClears,1,'Temporary paper surface retired at completion');
  ...
  s=await setup({preview:false});
  assert.ok(s.draws.some(state=>state.actors.length===0),'Backing store prewarmed before ready');
  const reads=s.stage.measures;
  const view={g:{width:390,height:844,left:0,viewportWidth:390,actorWidth:math.actorWidth(390)},rect:{left:0,top:0,width:390,height:585}};
  assert.ok(s.win.WeddingCameraStory.start({view,timestamp:3800}));
  assert.equal(s.stage.measures,reads,'No layout read at handoff');
  const writes=s.stage.writes;s.tick(3816);
  assert.equal(s.draws.at(-1).t,16,'Second stage inherits the same frame clock');
  assert.equal(s.stage.writes,writes,'No repeated unchanged style writes');
  s.win.WeddingCameraStory.complete();
  s=await setup({scrollX:12});s.win.scrollY=915;s.win.WeddingCameraStory.complete();
  assert.equal(s.scrolls.length,1);assert.equal(s.scrolls[0].left,0);assert.equal(s.scrolls[0].top,915,'Horizontal cleanup never jumps vertically');
  s=await setup({scrollX:12,zoom:2});s.win.WeddingCameraStory.complete();assert.equal(s.scrolls.length,0,'Preserve pinch-zoom position');
