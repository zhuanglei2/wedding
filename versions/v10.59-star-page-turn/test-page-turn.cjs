...
function setup(opts={}){
  ...
  const nodes={};
  for(const key of ['.opening-sequence','.turn-sheet','.image-cover','.page-turn-stars','#our-story','.turn-cue','.opening-underlay img'])nodes[key]=new El();
  ...
  const context={window:win,document:{documentElement:root,querySelector:k=>nodes[k]}};
  ...
  flush();
  return{nodes,win,root,reduce,queue,events,flush,scroll:y=>{win.scrollY=y;(events.scroll||[]).forEach(e=>e.fn());flush();}};
}
...
s.scroll(130+844*1.15*.55);assert.notEqual(s.nodes['.page-turn-stars'].style.opacity,'0.0000');
const pose=s.nodes['.turn-sheet'].style.transform;s.scroll(1100);s.scroll(130+844*1.15*.55);assert.equal(s.nodes['.turn-sheet'].style.transform,pose);
s.scroll(0);assert.equal(s.nodes['.page-turn-stars'].style.opacity,'0.0000');
let prevented=false;s.nodes['.turn-cue'].emit('click',{preventDefault:()=>{prevented=true;}});assert.ok(prevented);s.flush(0);s.flush(1300);assert.ok(s.win.scrollY>0&&s.win.scrollY<1101);s.flush(2600);assert.equal(s.win.lastScroll.behavior,'auto');assert.ok(s.nodes['#our-story'].focused);
...
console.log('PASS: invitation-first, staged flight/turn/exit, grip synchronization, exact rewind, viewport math, passive gestures, CTA, reduced motion and asset failures');
