...
const base=path.resolve(__dirname,'../v10.159-flow-and-gilt'),html=fs.readFileSync(path.join(__dirname,'index.html'),'utf8'),old=fs.readFileSync(path.join(base,'index.html'),'utf8');
...
let expected=old.replaceAll('V10.159','V10.160');
for(const file of ['reactions.css','flow-and-gilt.css','opening-runtime.js','handwriting.js'])expected=expected.replaceAll('"'+file+'"','"../v10.159-flow-and-gilt/'+file+'"');
expected=expected.replaceAll('"media/','"../v10.159-flow-and-gilt/media/');
assert.equal(html.trimEnd(),expected.trimEnd(),'only version and relative asset links change in HTML; visual layout unchanged');
console.log('PASS: '+references.size+' local resource references; inline and external JS syntax; unchanged markup, pictures, fonts and layout.');
