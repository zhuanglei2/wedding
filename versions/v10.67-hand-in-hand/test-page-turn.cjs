...
for(const width of [320,390,768,1000]){
  ...
  const renderedHand=(a,left)=>{
    ...
    const point=math.rotate(root.x+tip.x,root.y+tip.y,a.lean);
...
s.flush(math.DURATION+250);s.flush(math.DURATION+266);assert.ok(s.nodes['#our-story'].focused);assert.equal(s.win.scrollY,70+844*1.15);assert.equal(s.queue.size,0);
