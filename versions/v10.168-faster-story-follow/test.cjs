...
async function fixture({reduced=false,noWAAPI=false,broken=false,paperHeight=850,screenHeight=850,introHeight=280,wrap=18,rowHeight=28,scrollQuantum=0,frameStep=32,sourceOverride=null,handoffPending=false,integrated=false}={}){
 ...
 if(integrated){
  ...
  vm.runInNewContext(fs.readFileSync(path.join(__dirname,'../v10.166-mobile-scroll-owner/memory-handoff.js'),'utf8'),context);
 }
...
(async()=>{
 ...
 const recentRuntime=fs.readFileSync(path.join(__dirname,'../v10.167-larger-memory-photos/memory.js'),'utf8');
 for(const frameStep of [8,16,32]){
  ...
  const oldFollow=await followError(previousRuntime),newFollow=await followError(source),recentFollow=await followError(recentRuntime);
  assert.equal(newFollow.typed,recentFollow.typed,'typing unchanged versus V10.167');
  assert(newFollow.lag<recentFollow.lag*.85,'tracking lag decreases versus V10.167 at '+frameStep+'ms');
  console.log('V10.167 -> V10.168 mean following lag at '+frameStep+'ms: '+(recentFollow.lag/recentFollow.samples).toFixed(2)+'px -> '+(newFollow.lag/newFollow.samples).toFixed(2)+'px');
 ...
 const oldReturn=await firstResumedGlyph(previousRuntime),newReturn=await firstResumedGlyph(source),recentReturn=await firstResumedGlyph(recentRuntime);
 assert.equal(newReturn,recentReturn,'manual quiet time and finite rejoin unchanged from V10.167');
...
console.log('PASS: V10.159 freeze reproduced; V10.168 preserves the V10.160 fix with integer/half/third-pixel scrolling, 8/16/32ms frames, both directions, clamped scroll bounds and photo flights.');
