...
(()=>{'use strict';
 ...
 function photoPose(pose,cardWidth,cardHeight,width,height){
  const [x,y,r]=pose,angle=Math.abs(r)*Math.PI/180;
  // Allow for the existing 1.012 landing bounce and the viewport's faded edges.
  const halfWidth=(cardWidth*Math.cos(angle)+cardHeight*Math.sin(angle))*1.012/2;
  const halfHeight=(cardHeight*Math.cos(angle)+cardWidth*Math.sin(angle))*1.012/2;
  const roomX=Math.max(0,width/2-halfWidth-8),roomY=Math.max(0,height/2-halfHeight-16);
  return [clamp(x,-roomX/cardWidth*100,roomX/cardWidth*100),clamp(y,-roomY/cardHeight*100,roomY/cardHeight*100),r];
 }
 function buildFlights(){
  ...
  cards.forEach((card,i)=>{const step=schedule[i],[x,y,r]=photoPose(step.pose,card.offsetWidth||card.clientWidth,card.offsetHeight,width,box.height);const rest=`translate(calc(-50% + ${x}%),calc(-50% + ${y}%)) rotate(${r}deg)`;
 ...
 function tick(now){frame=0;if(!canPlay()){lastTime=null;return}const dt=lastTime===null?0:Math.min(64,now-lastTime);lastTime=now;
  ...
  if(!waitingForPhotos)elapsed+=dt;
   ...
   if(follow&&scrollTarget!==null){const distance=scrollTarget-port.scrollTop;if(Math.abs(distance)<1)scrollTarget=null;else port.scrollTop+=distance*Math.min(1,dt/70)}
   keepLineVisible();
   renderPhotos();
