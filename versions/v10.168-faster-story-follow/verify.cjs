...
const base=path.resolve(__dirname,'../v10.167-larger-memory-photos'),html=fs.readFileSync(path.join(__dirname,'index.html'),'utf8'),old=fs.readFileSync(path.join(base,'index.html'),'utf8');
...
const expected=old.replaceAll('V10.167','V10.168').replace('href="larger-photos.css"','href="../v10.167-larger-memory-photos/larger-photos.css"');
assert.equal(html.trimEnd(),expected.trimEnd(),'only version and inherited stylesheet URL differ');
const runtime=read(path.join(__dirname,'memory.js')),previous=read(path.join(base,'memory.js'));
assert.equal(runtime.trimEnd(),previous.replace('dt/95','dt/70').trimEnd(),'only the inner scroll follow coefficient changes');
const start=runtime.indexOf(' function photoPose('),end=runtime.indexOf(' function buildFlights(){');
const css=read(path.join(__dirname,'../v10.167-larger-memory-photos/larger-photos.css'));
