...
async function main(){
 ...
 const names=['groom','bride'],positions=[{right:8,top:1050,height:270},{left:1145,top:1340,height:270}],layers=[],layout=[];
 ...
 await sharp(output).extract({left:1030,top:970,width:370,height:720}).png().toFile(path.join(__dirname,'contact-detail.png'));
 ...
 let changedOutsideOverlay=0,changedBody=0,changedBorder=0,changedContent=0;
 for(let y=0;y<3282;y++)for(let x=0;x<1400;x++){
  ...
  if(diff&&y>=1650)changedBody++;
  if(diff&&x<1138)changedContent++;
  if(diff&&(x===0||x===1399))changedBorder++;
 }
 assert.equal(changedOutsideOverlay,0);assert.equal(changedBody,0);assert.equal(changedBorder,0);assert.equal(changedContent,0);
 assert.equal(hash(coverPath),originals.cover);assert.equal(hash(actorsPath),originals.actors);
 const report={version:'10.83',kind:'static_original_cover_plus_character_layers',width:1400,height:3282,originals,layout,checks:{sourceFilesUnchanged:true,changedOutsideOverlay,changedAtOrBelow1650px:changedBody,changedProtectedContentPixels:changedContent,changedSideBorderPixels:changedBorder,originalAspectPreserved:true,noAddedMargins:true,noPrecurledPaper:true},limits:['Static pose only: contact and natural motion not validated.','Full cover is taller than a 9:16 viewport; no promise of 9:16 video compatibility.','Characters are from V10.80 pose draft, not pixel-identical to original product photo.','No real-video matte/mesh integration, no upload, no deployment.']};
